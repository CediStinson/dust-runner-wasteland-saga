...
const DiaryModal: React.FC<DiaryModalProps> = ({ showDiary, setShowDiary, diaryEntries = [] }) => {
  ...
  const finalEntries = diaryEntries.length > 0 ? diaryEntries : defaultEntries;
  const totalPages = finalEntries.length;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-auto">
      ...
      <div className="relative bg-stone-200 w-[90%] max-w-2xl h-[80%] rounded-lg shadow-lg overflow-hidden">
        ...
        <div className="bg-stone-300 p-4 border-b border-stone-400">
          ...
          <p className="text-stone-600 text-sm">Pages found: {finalEntries.filter(entry => entry).length}/{totalPages}</p>
        </div>
        ...
        <div className="p-8 h-[calc(100%-8rem)] overflow-auto flex items-center justify-center">
          <div className="w-full max-w-md min-h-[400px] bg-stone-100 p-6 shadow-inner rounded border border-stone-300 flex flex-col">
            {/* Page content */}
            {finalEntries[currentPage] ? (
              <p className="text-stone-800 font-serif leading-relaxed flex-1">{finalEntries[currentPage]}</p>
            ) : (
