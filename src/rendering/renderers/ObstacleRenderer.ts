...
export default class ObstacleRenderer {
  ...
  drawTarp(obs: ObstacleObject) {
    ...
    this.p.rect(0, 0, obs.width || 0, obs.height || 0, 8); // Rounded rectangle for the tarp
    ...
    // Draw wrinkles
    for (let i = 0; i < 8; i++) {
      const y = i * ((obs.height || 0) / 8) + this.p.random(-3, 3);
      const waveAmplitude = this.p.random(2, 5);
      
      this.p.beginShape();
      for (let x = 0; x < (obs.width || 0); x += 5) {
    ...
    if (obs.holePositions) {
      for (const hole of obs.holePositions) {
        const holeX = hole.x * (obs.width || 0);
        const holeY = hole.y * (obs.height || 0);
    ...
    this.p.rect(5, 5, obs.width || 0, obs.height || 0, 8);
  ...
  drawRock(obs: ObstacleObject) {
    ...
    let shadowHeight = 20 * obs.size * (obs.aspectRatio && obs.aspectRatio < 1 ? 1 / this.p.abs(obs.aspectRatio) : 1);
